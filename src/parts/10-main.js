...
(() => {
  ...
  class SettingsValidator {
    sanitize(rawSettings = {}) {
      const normalizeStringList = (value) => Utils.toStringArray(value).map((entry) => Utils.normalizeText(entry));

      const maxPrice = Math.max(0, Number(rawSettings.maxPrice) || 0);
      const hideColdDeals = Boolean(rawSettings.hideColdDeals);
      const debugEnabled = Boolean(rawSettings.debugEnabled);
      const hideMatchingMerchantNames = Boolean(rawSettings.hideMatchingMerchantNames);

      const excludeWords = Utils.toStringArray(rawSettings.excludeWords);
      const whitelistWords = Utils.toStringArray(rawSettings.whitelistWords);
      const blockedUsers = normalizeStringList(rawSettings.blockedUsers);
      const hiddenDeals = Utils.toStringArray(rawSettings.hiddenDeals);

      const excludeMerchantsData = Array.isArray(rawSettings.excludeMerchantsData)
        ? rawSettings.excludeMerchantsData
  ...
  class SettingsStore {
    constructor(storageApi, validator = new SettingsValidator()) {
      this.storageApi = storageApi;
      this.validator = validator;
      this.onRemoteChange = null;
      this.settings = this.validator.sanitize(this.load());
    ...
    setupSyncListener() {
      ...
      Object.values(CONFIG.storageKeys).forEach((key) => {
        GM_addValueChangeListener(key, (_name, _oldValue, _newValue, remote) => {
          if (!remote) return;
          this.settings = this.validator.sanitize(this.load());
          if (typeof this.onRemoteChange === "function") {
            this.onRemoteChange();
          }
        });
      });
    ...
    setRemoteChangeHandler(handler) {
      this.onRemoteChange = typeof handler === "function" ? handler : null;
    }

    save() {
      this.settings = this.validator.sanitize(this.settings);
    ...
    matchesAnyFilter(filters, normalizedTitle) {
      return filters.some((filter) => {
        try {
          ...
          const isTimedOut = () => performance.now() - start > 50;

          const includesAll = filter.includeRegex.every((regex) => {
            if (isTimedOut()) throw new Error("Filter timeout");
            return regex.test(normalizedTitle);
          });

          if (!includesAll) return false;

          const excludesMatch = filter.excludeRegex.some((regex) => {
            if (isTimedOut()) throw new Error("Filter timeout");
            return regex.test(normalizedTitle);
          });
          ...
          this.logger.warn("Filterauswertung übersprungen", filter.raw, error.message);
          return false;
        }
      });
    }
  ...
  class UiController {
    ...
    openSettings() {
      ...
      overlay.setAttribute("aria-labelledby", "mdm-modal-title");

      overlay.innerHTML = `
        <div class="mdm-modal" role="dialog" aria-modal="true">
          <h3 id="mdm-modal-title">mydealz Manager – Einstellungen</h3>
          <div class="mdm-grid">
            <div>
  ...
  class MyDealzManagerApp {
    constructor() {
      ...
      this.settingsStore.setRemoteChangeHandler(() => {
        this.filterEngine.refreshCompiledFilters();
        this.processDeals();
      });
    ...
    installMutationObserver() {
      ...
      this.observer = new MutationObserver((mutations) => {
        ...
        const hasRelevantChanges = mutations.some((mutation) => {
          ...
          return Array.from(mutation.addedNodes).some((node) => {
            ...
            return (
              node.matches?.(CONFIG.selectors.deal) ||
              node.querySelector?.(CONFIG.selectors.deal) ||
              node.matches?.(CONFIG.selectors.subNavFilter) ||
              node.querySelector?.(CONFIG.selectors.subNavFilter)
            );
          });
        });
      ...
      const dealContainer = document.querySelector('[data-t="threadGrid"]') || document.body;
      this.observer.observe(dealContainer, {
        childList: true,
        subtree: true,
      });
    }
    ...
    getDisplayTitle(article, baseTitle, merchantName) {
      ...
      try {
        const merchantPattern = new RegExp(`\b${Utils.escapeRegExp(merchantName.trim())}\b`, "ig");
        const cleaned = original
          .replace(merchantPattern, "")
          .replace(/\s{2,}/g, " ")
          .replace(/^\s*[|\-:]\s*/, "")
          .trim();
        ...
        this.logger.error("Händlername konnte nicht aus Titel entfernt werden", error);
        titleNode.textContent = original;
        return original;
      }
    }
